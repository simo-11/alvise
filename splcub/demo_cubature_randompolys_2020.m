...
function demo_cubature_randompolys_2020
...
example=2;     % experiments: choose from 0 to 4 (see the attached file
...
number_experiments=100; % number of tests for each degree.
...
for ii=1:length(adeV)
    ...
    momerr_splcub(ii)=norm(cmom-moms_splcub,inf);
    momerr_splgss(ii)=norm(cmom-moms_splgss,inf);
    ...
    for jj=1:number_experiments
        ...
        % ............. reference result ............
        I_exact = exact_integral(c1,k1,k2,n,Sx,Sy);
        ...
        % ... Absolute error ...
        aeV(jj)=abs(I_exact-I_2019(end));
        
        % ... Absolute error ...
        aeV_2009(jj)=abs(I_exact-I_2009(end));
        
        deltaVRE(jj)=abs(I_2019-I_2009(end));
        
        % ... Relative error ...
        if abs(I_exact) > 10^(-6) | abs(I_exact) < 10^(+12)
            reV(jj)=abs(I_exact-I_2019(end))/abs(I_exact);
            reV_2009(jj)=abs(I_exact-I_2009(end))/abs(I_exact);
        else
            reV(jj)=0;
            reV_2009(jj)=0;
        end
    ...
    fprintf('\n \t * DEG: %2.0f RE 2020: %1.1e RE 2009: %1.1e DELTA RE: %1.1e',...
        n,aeVmax(ii),aeVmax_2009(ii),deltaVmaxRE(ii));
    ...
    % root mean squares
    RMS_abs_2009(ii)=rms(log10(aeV_2009(iok3))-mean(log10(aeV_2009(iok3))));
    RMS_rel_2009(ii)=rms(log10(reV_2009(iok4))-mean(log10(reV_2009(iok4))));
    
    
    % plots
    plot_errors(ii,n,reV,log_reV(ii));
...
fprintf('\n \t 2020');
fprintf('\n  ----------------------------------------------------------------------------');
fprintf('\n  |  ade |  max(ae) |  max(re) | max*(ae) |  max*(re) |  ARMSE    |  RRMSE   |');
fprintf('\n  ----------------------------------------------------------------------------');
...
fprintf('\n  max*(ae) : average absolute error');
fprintf('\n  max*(re) : average relative error');
fprintf('\n  ARMSE    : RMSE absolute error');
fprintf('\n  RRMSE    : AMSE absolute error');
fprintf('\n  ----------------------------------------------------------------------------');
...
for ii=1:length(adeV)
    fprintf('\n  | %3.0f  | %1.2e | %1.2e | %1.2e | %1.2e  | %1.2e  | %1.2e |',...
        ...
        RMS_abs_2009(ii),RMS_rel_2009(ii));
end
...
fprintf('\n  ----------------------------------------------------------------------------');
fprintf('\n  ade      : algebraic degree of precision');
fprintf('\n  moms_spc : max momerr splcub');
fprintf('\n  moms_spg : max momerr splgauss');
fprintf('\n  ----------------------------------------------------------------------------');
