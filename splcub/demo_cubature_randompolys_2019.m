...
function demo_cubature_randompolys_2019
...
for ii=1:length(adeV)
    ...
    for jj=1:100
        ...
        rect=[min(Zin(:,1)) max(Zin(:,1)) min(Zin(:,2)) max(Zin(:,2))];
        ...
        % ... Absolute error ...
        aeV(jj)=abs(I_2009-I_2019(end));
        
        % ... Relative error ...
        if abs(I_2009) > 10^(-6), reV(jj)=abs(I_2009-I_2019(end))/...
                abs(I_2009);
        else reV(jj)=0; end
        
        % ... Statistics purging small integrals ...
        if abs(I_2009) < 10^(-9)
            purged_aeV(jj)=0;
            purged_reV(jj)=0;
        else
            purged_aeV(jj)=aeV(jj);
            purged_reV(jj)=reV(jj);
        end
    ...
    purged_aeVmax(ii)=max(purged_aeV);
    purged_reVmax(ii)=max(purged_reV);
...
fprintf('\n \n')
fprintf('\n  -----------------------------------------------------');
fprintf('\n  |  ade |  max(ae) |  max(re) | max*(ae) |  max*(re) |');
fprintf('\n  -----------------------------------------------------');

% statistics: experiments
for ii=1:length(adeV)
    fprintf('\n  | %3.0f  | %1.2e | %1.2e | %1.2e | %1.2e |',...
        adeV(ii), aeVmax(ii),reVmax(ii),purged_aeVmax(ii),purged_reVmax(ii));
end
...
fprintf('\n  max*(ae)  : worst absolute error (purging small integrals)');
fprintf('\n  max*(re)  : worst relative error (purging small integrals)');
fprintf('\n  -----------------------------------------------------------');
...
% INPUT:
%
% [x,y]: EVALUATE THE FUNCTION IN THE POINTS (x,y). "x", "y" ARE COLUMN
%        VECTORS
%
% OUTPUT:
%
