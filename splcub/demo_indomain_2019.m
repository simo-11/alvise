...
function demo_indomain_2019

% This demo:
% * tests the performance of the indomain routine
% * plots domain and poinsets

%============================= experiment data ============================

example=1;     % experiments: choose from 0 to 4 (see the attached file
               %  define_example.m); for those in the paper, 1 or 2.

nV=10.^(2:6);   % cardinalities
...
% rough determination of a rectangle containing the domain
X=polygon_sides(:,1); Y=polygon_sides(:,2); 
xm=min(X); xM=max(X); ym=min(Y); yM=max(Y);
xm=min(0.8*xm,1.2*xm); xM=max(0.8*xM,1.2*xM);
ym=min(0.8*ym,1.2*ym); yM=max(0.8*yM,1.2*yM);

% perform experiment for ADE equal to "nV"
for ii=1:length(nV)

    n=nV(ii);
    % pointset to test in rectangle R^*
    P=[xm+(xM-xm)*rand(n,1) ym+(yM-ym)*rand(n,2)];
    
    % inpolygon
    tic;
    [in,on]=incurvpolygon(P,Sx,Sy,[],[],SPLtypestring);
    CPU_in(ii)=toc;
    ...
    figure(ii)
    iin=find(in == 1);
    plot_spl(Sx,Sy,[],P,P(iin,:)); % figure
    titlestr=strcat('Cardinality: ',num2str(n));  % title
