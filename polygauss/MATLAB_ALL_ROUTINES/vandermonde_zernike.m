...
function V=vandermonde_zernike(pts,deg)

% OBJECT:
% Vandermonde Matrix of Zernike basis (suitable for interpolation)
% Notice that this is ANSI numeration of Zernike polynomials.

% INPUT:
% pts: N x 2 matrix of points
% deg: higher degree of the matrix
%
% OUTPUT:
% V: let "dim=(deg+1)*(deg+2)/2" and N the number of points. It is a 
%    N x dim matrix.
%
% EXAMPLE:
% >> V=vandermonde_zernike([0 0],2)
%   V =
%    0.5642         0         0         0   -0.9772         0
% >> 


[thz,rz]=cart2pol(pts(:,1),pts(:,2));

V=[];

for n=0:deg
    for j=-n:2:n
        fx=zernfun(n,j,rz,thz,'norm');
        % fx=zernfun(n,j,rz,thz);
        V=[V fx];
    end
end
